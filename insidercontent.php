...
<div class="panel panel-primary">
  <div class="panel-body list-group-item active">New Items in <?php echo $cname; ?></div>
  <div class="panel-footer">
	...
	<?php 
					...
					$sql = "SELECT * FROM videos WHERE subcat_id='$subcat_id' ORDER BY vid_id DESC LIMIT $start_from, $num_rec_per_page"; 
			  ...
			  <div class="list-group-item"><table style="width:100%"><tbody><tr><th></th><td width="50%"><span class="badge">Rs. '.$row['vid_price'].'</span></a></td><td width="50%"><b><span class="glyphicon glyphicon-thumbs-up"></span> </b>'.$row['likes'].'</td></tr></tbody></table></div>
			   <div class="list-group-item"><table style="width:100%"><tbody><tr><th></th><td><a href="showproduct.php?id='.$row['vid_id'].'" class="btn btn-default btn-block">Buy Now</a></td></tr></tbody></table></div>
						...
						echo '<center>No Item in this category.</center>';
					}
				?>
	<?php
	...
	*/
	...
	<div class="row">
		<center>
			<ul class="pagination">
				<?php 
					$sql = "SELECT * FROM videos WHERE subcat_id='$subcat_id' ORDER BY vid_id DESC"; 
					$rs_result = $conn->query($sql); //run the query
					$total_records = $rs_result->num_rows;  //count number of records
					$total_pages = ceil($total_records / $num_rec_per_page); 
					
					if($total_records>0)
					echo "<li><a href='viewinsider.php?subcat=".$subcat_id."&page=1'>".'<'."</a></li> "; // Goto 1st page  

					for ($i=1; $i<=$total_pages; $i++) { 
								echo "<li><a href='viewinsider.php?subcat=".$subcat_id."&page=".$i."'>".$i."</a></li> "; 
					}; 
					if($total_records>0)
					echo "<li><a href='viewinsider.php?subcat=".$subcat_id."&page=$total_pages'>".'>'."</a></li> "; // Goto last page
				?>
			</ul></center>
</div>
