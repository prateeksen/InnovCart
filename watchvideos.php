...
<div class="panel panel-primary">
  <div class="panel-body list-group-item active"><center><span class="glyphicon glyphicon-film"></span> Watched Products</center></div>
  <div class="panel-footer">
	...
	<?php
				$suser=$_SESSION['username'];
					...
					$sql="SELECT * FROM videos,member_watch WHERE watch_vid=vid_id and watch_by='$suser' ORDER BY watch_time DESC LIMIT $start_from, $num_rec_per_page"; 
	...
	<div class="row">
		<center>
			<ul class="pagination">
				<?php 
					$sql = "SELECT * FROM videos,member_watch WHERE watch_vid=vid_id and watch_by='$suser' ORDER BY watch_time DESC"; 
					$rs_result = $conn->query($sql); //run the query
					$total_records = $rs_result->num_rows;  //count number of records
					$total_pages = ceil($total_records / $num_rec_per_page); 
					
					if($total_records>0)
					echo "<li><a href='viewhistory.php?&page=1'>".'<'."</a></li> "; // Goto 1st page  

					for ($i=1; $i<=$total_pages; $i++) { 
								echo "<li><a href='viewhistory.php?&page=".$i."'>".$i."</a></li> "; 
					}; 
					if($total_records>0)
					echo "<li><a href='viewhistory.php?&page=$total_pages'>".'>'."</a></li> "; // Goto last page
				?>
			</ul></center>
</div>
